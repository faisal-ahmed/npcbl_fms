<section id="contact-page" style="padding: 50px 0;">
    <div class="container-fluid" style="padding: 0 3% 0 10% !important;">
        <div class="row">
            <div class="col-sm-12">
                <h2 class="text-center"><i class="fas fa-file-medical text-primary"></i> Apply for Other Leave</h2>
            </div>
        </div>
        ...
        <div class="content-background">
            ...
            <div class="row wow fadeInDown">
                <div class="col-sm-12">
                    ...
                    <div class="row">
                        <div class="col-sm-6 form-group">
                            ...
                            <input type="text" class="form-control" disabled value="<?php echo esc($userData['Name_English'] ?? ''); ?>">
                        </div>
                        <div class="col-sm-6 form-group">
                            <label>Full Name (Bangla)</label>
                            <input type="text" class="form-control" disabled value="<?php echo esc($userData['Name_Bangla'] ?? ''); ?>">
                        </div>
                    </div>
                    <div class="row">
                        <div class="col-sm-6 form-group">
                            <label>Date of Joining</label>
                            <input type="text" class="form-control" disabled value="<?php echo esc($userData['Joining_Date'] ?? ''); ?>">
                        </div>
                        <div class="col-sm-6 form-group">
                            <label>Joining Department</label>
                            <input type="text" class="form-control" disabled value="<?php echo esc($userData['Joining_Department'] ?? ''); ?>">
                        </div>
                    </div>
                    <div class="row">
                        <div class="col-sm-6 form-group">
                ...
                <div class="col-sm-12">
                    <form class="apply-other-form" method="post" action="<?php echo base_url('leave/apply-for-others'); ?>">
                        ...
                        <div class="row">
                            <div class="col-sm-4 form-group">
                                ...
                                <select name="leave_type" id="leave_type" class="form-control" required style="height: 48px;">
                                    <option value="">-- Select Leave Type --</option>
                                    <?php if (isset($leaveType)) : ?>
                                        <?php foreach($leaveType as $leave): ?>
                                            <?php if ($leave['type_id'] != CASUAL_LEAVE_ID) : ?>
                                                <option value="<?php echo esc($leave['type_id']); ?>" <?php echo set_select('leave_type', $leave['type_id'], (isset($formData['leave_type']) && $formData['leave_type'] == $leave['type_id'])); ?>>
                                                    <?php echo esc($leave['type_name']); ?>
                                                </option>
                                            <?php endif; ?>
                                        <?php endforeach; ?>
                                    <?php endif; ?>
                            ...
                            <div class="col-sm-4 form-group">
                                <label for="remaining_days_display">Balance Remaining</label>
                                <input type="text" id="remaining_days_display" class="form-control"
                                       style="height: 48px; font-weight: bold; color: #c52d2f;"
                                       disabled value="0 Days">
                            </div>

                            <div class="col-sm-4 form-group">
                                <label for="total_days">Total No. of Days Requested  <span class="text-danger">*</span></label>
                                <input type="number" name="total_days" id="total_days" style="height: 48px;" class="form-control" readonly value="<?php echo set_value('total_days'); ?>" required>
                            </div>
                        </div>
                        ...
                        <div class="row">
                            <div class="col-sm-4 form-group">
                                ...
                                <input type="date" name="start_date" id="start_date" style="height: 48px;" class="form-control" required
                            ...
                            <div class="col-sm-4 form-group">
                                ...
                                <input type="date" name="end_date" id="end_date" style="height: 48px;" class="form-control" required
...
<script type="text/javascript">
    const leaveBalances = <?php echo json_encode($balances); ?>;
    const leaveTypeSelect = document.getElementById('leave_type');
    const remainingInput = document.getElementById('remaining_days_display');
    const startInput = document.getElementById('start_date');
    const endInput = document.getElementById('end_date');
    const totalDaysInput = document.getElementById('total_days');
    const form = document.querySelector('.apply-other-form');

    function updateBalanceDisplay() {
        const selectedType = leaveTypeSelect.value;
        if (selectedType && leaveBalances[selectedType]) {
            const bal = leaveBalances[selectedType];
            // Accessing the clean 'remaining' key from formatted balances
            const remaining = parseInt(bal.remaining) || 0;
            remainingInput.value = remaining + " Days";
            remainingInput.dataset.val = remaining;
        } else {
            remainingInput.value = "0 Days";
            remainingInput.dataset.val = 0;
        }
    }
    ...
    if (leaveTypeSelect.value) { updateBalanceDisplay(); }

    form.addEventListener('submit', function (e) {
        const requested = parseInt(totalDaysInput.value) || 0;
        const available = parseInt(remainingInput.dataset.val) || 0;
        ...
        if (requested > available) {
            alert("Insufficient Balance! You requested " + requested + " days, but you only have " + available + " days remaining.");
            e.preventDefault();
            return false;
        }
